...
def getDplrParameters(gmm_params, S, epsilon, use_precision=False):
    '''
    Converts full covariance matrices into the DPLR matrix structure described
    in [1]. The model parameters returned by this function can be readily used
    as initial parameters for GMMClassifier
    ...
    for c in range(C):
        ...
        if use_precision:
            for k in range(K[c]):
                ...
                init_d_rho[:, k_idx] = np.log(np.exp(d - max_d_0) - np.exp(0 - max_d_0)) + max_d_0
        else:
            for k in range(K[c]):
                k_idx = k1 + k
